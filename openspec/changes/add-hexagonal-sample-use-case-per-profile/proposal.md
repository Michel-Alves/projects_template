...
- `kotlin-microservice-relational-db-profile`: The existing sample `@Entity` + `JpaRepository` becomes the relational outbound adapter for the sample use case, implementing the domain port rather than being consumed directly by a controller.
