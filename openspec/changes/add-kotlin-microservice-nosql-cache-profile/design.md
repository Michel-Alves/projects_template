...
- Use the **raw MongoDB Kotlin sync driver** (`mongodb-driver-kotlin-sync`) with a template-owned `MongoConfig` that builds `MongoClient` + `MongoDatabase` beans from `app.mongo.*` properties. No Spring Data Mongo on the classpath.
...
- **Choice**: Depend on `mongodb-driver-kotlin-sync` directly; application code uses `MongoClient` / `MongoDatabase` / `MongoCollection` APIs. For Redis, use `spring-boot-starter-data-redis` with Lettuce and `RedisTemplate`.
- **Why (Mongo side)**: Spring Data MongoDB is a heavy abstraction — `MongoTemplate`, repository interfaces, custom converters, index-auto-creation, query derivation. Most teams end up fighting it for non-trivial queries. The raw Kotlin sync driver is small, idiomatic, well-documented, and keeps the template honest about what "using MongoDB" actually looks like.
...
### D5. `MongoConfig` builds `MongoClient` + `MongoDatabase` beans with the POJO codec registry
- **Choice**: A `@Configuration` class registers:
  - `MongoClient` from `app.mongo.uri` using `MongoClients.create(settings)` where `settings` is a `MongoClientSettings` built with the default codec registry plus the POJO codec registry (`PojoCodecProvider.builder().automatic(true).build()`). Closing is lifecycle-managed via `@Bean(destroyMethod = "close")`.
  - `MongoDatabase` from `mongoClient.getDatabase(app.mongo.database)`.
- **Why**: The POJO codec registry is what lets `MongoCollection<SampleDocument>` serialize/deserialize plain Kotlin data classes without per-field manual `Document` conversion. It's the idiomatic raw-driver pattern for structured documents.
- **Kotlin data class caveat**: The automatic POJO codec needs a no-arg constructor for deserialization, which Kotlin data classes do not emit by default. Options: (a) give each field a default value (one-liner, keeps `data class`), (b) apply the `kotlin-noarg` Gradle plugin to generate synthetic no-arg constructors for documents annotated with a marker, (c) use `kotlinx.serialization` with the `bson-kotlinx` module. Defer the final pick to the sample implementation — leaning toward (a) for minimum new moving parts; the sample only has one document and giving `id: String = ""` / `name: String = ""` defaults is trivial.
- **Alternatives considered**: Manual `Document` conversion (noisy, unergonomic), `bson-kotlinx` + `kotlinx.serialization` (more powerful but pulls another serialization framework into a template that today has none).
...
- **[Risk] Kotlin POJO codec needs no-arg constructor support; `data class` doesn't generate one by default.** → Mitigation: use default values on every field of `SampleDocument` (`id: String = ""`, `name: String = ""`). Document this gotcha in the template's README so downstream teams know why. If defaults are unacceptable for their documents they can add the `kotlin-noarg` plugin.
...
- **Exact Mongo Kotlin driver version?** The Kotlin sync driver lives under `org.mongodb:mongodb-driver-kotlin-sync`. Pin to the latest 5.x at apply time.
...
- **Kotlin data class no-arg strategy: defaults vs. `kotlin-noarg` plugin vs. `bson-kotlinx`?** Leaning defaults (D5). Revisit if the sample grows past one document.
