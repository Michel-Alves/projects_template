...
- The `nosql-cache` profile adds, on top of `default`:
  - **MongoDB Kotlin sync driver** (`org.mongodb:mongodb-driver-kotlin-sync`) — the raw driver, **not** Spring Data MongoDB. A small `MongoConfig` `@Configuration` registers a `MongoClient` and a `MongoDatabase` bean from properties; application code uses the driver API directly (no `MongoRepository`, no `MongoTemplate`).
...
- **Modified files in this repo**:
  - `kotlin-microservice/project.json` — append `nosql-cache` to the `stack_profile` array; pin `mongo_image_tag`, `redis_image_tag`, `mongock_version`, `mongo_driver_version` (Spring Boot's BOM manages Spring Data Redis / Lettuce versions; Mongo driver is pinned explicitly since it's no longer transitively managed by `spring-boot-starter-data-mongodb`).
  - `kotlin-microservice/template/build.gradle.kts` — extend the profile conditional with a `nosql-cache` branch adding `mongodb-driver-kotlin-sync`, `spring-boot-starter-data-redis`, `mongock-springboot-v3`, `mongodb-sync-v4-driver`, and Testcontainers `mongodb` + core modules.
...
- **New files in this repo** (only rendered when `stack_profile=nosql-cache`):
  - `kotlin-microservice/template/src/main/kotlin/{{tld}}/{{author}}/{{app_name}}/persistence/MongoConfig.kt` — `@Configuration` that builds `MongoClient` + `MongoDatabase` beans from `app.mongo.*` properties (POJO codec registry for Kotlin data classes).
  ...
  - `kotlin-microservice/template/src/main/kotlin/{{tld}}/{{author}}/{{app_name}}/persistence/SampleDocumentRepository.kt` — wraps a `MongoCollection<SampleDocument>` with small `findById`/`insert`/`deleteById` methods.
